function [RLRR_TE11] = GSM_v1_OPT(x, input_info)
...
if input_info.GEO.type == 1   
    f_cutoff = fc(x(1), 1, 1);
    f_cutofft = fc(x(input_info.GEO.N), 1, 1);
else
    f_cutoff = fc(x(1), 1, 1);
    f_cutofft = fc(x(input_info.GEO.N+1), 1, 1);
 end
    
        
    if f_cutoff(1) > (input_info.MODES.f_begin * 1e9)
        disp('The tool will discard this frequency : '); disp(input_info.MODES.f_begin); disp(' GHz'); 
        disp('Because it is evanascent for the first waveguide in the Geometry: ');
        disp('It will compute the optimization at a frequency which is close to the cut off frequency of first waveguide: ')
%         input_info.MODES.f_axis = linspace(f_cutoff(1), input_info.MODES.f_end * 1e9, input_info.MODES.Nf);
        input_info.MODES.f_axis = f_cutoff(1)+eps;
    else
%         input_info.MODES.f_axis = linspace(input_info.MODES.f_begin * 1e9, input_info.MODES.f_end * 1e9, input_info.MODES.Nf);
        input_info.MODES.f_axis = input_info.MODES.f_begin;
    end
...
%         mur(i) = input_info.GEO.E(i).mur;           
        R(i) = x(i);
        L(i) = x(i + input_info.GEO.N);
        er(i) = input_info.GEO.E(1).er;
        mur(i) = input_info.GEO.E(1).mur;
...
%        R_Cone_prev = input_info.GEO.E(i - 1).r;
       ...
       Len_Cone = x(i + input_info.GEO.N+1); % input_info.GEO.E(i).l;
       R_Cone = x(i); % input_info.GEO.E(i).r;
       R_Cone_prev = x(i - 1); %input_info.GEO.E(i - 1).r;
       
%        N_ele(i) = round(Len_Cone/dl);
       N_ele = round(Len_Cone/dl);
       
       R_intermediate = linspace(R_Cone_prev, R_Cone, N_ele + 1);
       ...
       er = [er ones(1, length(R_intermediate2)) * input_info.GEO.E(1).er];
       mur = [mur ones(1, length(R_intermediate2)) * input_info.GEO.E(1).mur];
