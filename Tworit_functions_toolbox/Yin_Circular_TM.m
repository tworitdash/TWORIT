%% Mutual admittance function for modes like TM1m and TM1n for a clyndrical waveguide

% Inputs:
%       m:          - Second mode index of first TM mode TM1m
%       n:          - second mode index of second TM mode TM1n
%       K0:         - Wavenumber of operation
%       R:          - Radius of the waveguide
%       er:         - Relative permittivity
%       mur:        - Relative permeability
%       L:          - Integration limit in multiples of K0
%Outputs:
%       Yin:        - Mutual reactance between mode TM1m and TM1n
...
function [Yin] = Yin_Circular_TM(m, n, k0, R, er, mur, L) 
...
% % 
Nup = (pi*(1+deltam)/2 .* (xmn).^2 .* (besselj_der(1, xmn)).^2).^(-1);
Nup1 = (pi*(1+deltam)/2 .* (xmn1).^2 .* (besselj_der(1, xmn1)).^2).^(-1);
...
I =  @(Nu) sqrt(Nup) .* sqrt(Nup1) .* beta_rhop .* beta_rhop1 .* pi ./ (4 .* omega .* epsilon .* kz(Nu)) .* ((2 .* k0.^2 - Nu.^2).*...
(I0(Nu) .* I0_2(Nu) + I2(Nu) .* I2_2(Nu)) - 1./2 .* Nu.^2 .* (I0(Nu) .* I2_2(Nu) + 3 .* I0_2(Nu) .* I2(Nu))) .* Nu ;
...
I_near_betarho = @(Nu) sqrt(Nup) .* sqrt(Nup1) .* beta_rhop .* beta_rhop1 .* pi ./ (4 .* omega .* epsilon .* kz(Nu)) .* ((2 .* k0.^2 - Nu.^2).*...
(I0_(Nu) .* I0_(Nu) + I2_(Nu) .* I2_(Nu)) - 1./2 .* Nu.^2 .* (I0_(Nu) .* I2_(Nu) + 3 .* I0_(Nu) .* I2_(Nu))) .* Nu ;
...
I_near_betarhomin = @(Nu) sqrt(Nup) .* sqrt(Nup1) .* beta_rhop .* beta_rhop1  .* pi ./ (4 .* omega .* epsilon .* kz(Nu)) .* ((2 .* k0.^2 - Nu.^2).*...
(I0_(Nu) .* I0_2(Nu) + I2_(Nu) .* I2_2(Nu)) - 1./2 .* Nu.^2 .* (I0_(Nu) .* I2_2(Nu) + 3 .* I0_2(Nu) .* I2_(Nu))) .* Nu ;

% I_near_betarhomin = @(Nu) sqrt(Nup) .* sqrt(Nup1) .* beta_rhop .* beta_rhop1  .* pi ./ (4 .* omega .* epsilon .* kz(Nu)) .* ((2 .* k0.^2 - Nu.^2).*...
% (I0_(Nu) .* I0_(Nu) + I2(Nu) .* I2_(Nu)) - 1/2 * Nu.^2 .* (I0_(Nu) .* I2_2(Nu) + I0_2(Nu) .* I2_(Nu))) .* Nu ;

I_near_betarhomax = @(Nu) sqrt(Nup) .* sqrt(Nup1) .* beta_rhop .* beta_rhop1  .* pi ./ (4 .* omega .* epsilon .* kz(Nu)) .* ((2 .* k0.^2 - Nu.^2).*...
(I0(Nu) .* I0_(Nu) + I2(Nu) .* I2_(Nu)) - 1./2 .* Nu.^2 .* (I0(Nu) .* I2_(Nu) + 3 .* I0_(Nu) .* I2(Nu))) .* Nu ;
